...
public partial class EntitySaveLoader
{
    ...
    {
        var savingEntities = Context<Game>.AllOf<SavingDataComponent>();//contexts.game.GetGroup(GameMatcher.SavingData).GetEntities();
    ...
    public string MakeEntityInfoJson(IEntity entity, Formatting jsonFormatting, string templateName)
    {
        ...
        var jsonStr = JsonConvert.SerializeObject(EntityTemplate, jsonFormatting, new JsonSerializerSettings()
        {
            ReferenceLoopHandling = Newtonsoft.Json.ReferenceLoopHandling.Ignore
        });
    ...
    private IEntity MakeEntityByContext(EntityTemplate EntityTemplate, Contexts contexts)
    {
        ...
        {
            case "Game":
                newEntity = contexts.GetContext<Game>().CreateEntity();
                break;
            /*case "Input":
